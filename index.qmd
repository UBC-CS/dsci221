...
Livestream <a href="https://canvas.ubc.ca/courses/176610/external_tools/61556"><img src="https://upload.wikimedia.org/wikipedia/commons/7/7b/Zoom_Communications_Logo.svg" alt="Zoom" style="height:12px; vertical-align:middle;"></a>
:::
:::::
...
schedule <- read_csv(here("data", "schedule.csv"), show_col_types = FALSE) |>
  ...
  mutate(
    mon_slides = if ("mon_slides" %in% names(cur_data())) mon_slides else NA_character_,
    tue_slides = if ("tue_slides" %in% names(cur_data())) tue_slides else NA_character_,
    wed_slides = if ("wed_slides" %in% names(cur_data())) wed_slides else NA_character_,
    weekly_summaries = if ("weekly_summaries" %in% names(cur_data())) weekly_summaries else NA_character_
  ) |>
