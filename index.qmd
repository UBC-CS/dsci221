...
schedule <- read_csv(
  ...
  mutate(
    ...
    examlet_display = case_when(
      ...
      !is.na(exam) ~ exam,
      # Practice material - use link from CSV if it's a markdown link, otherwise just show icon
      !is.na(exam_practice) & str_detect(exam_practice, "^\\[.*\\]\\(.*\\)$") ~
        str_replace(exam_practice, "^\\[.*\\]\\((.*)\\)$", glue("[{fa('pen-to-square')}](\\1)")),
